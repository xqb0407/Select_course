...
/**
* @author 作者 :Herther
* @version 创建时间：2020-12-20 23:52:42
* 类说明
*/
public class TeditGradelnternalFrame extends JInternalFrame {
	...
	public TeditGradelnternalFrame() {
		...
		GradeField.setFont(new Font("宋体", Font.BOLD, 20));
	...
	protected void editGrade(ActionEvent event) {
		...
		if(selectedRow== -1) {
			...
			JOptionPane.showMessageDialog(null, "成绩没有不能为空");
		}else {
	...
	protected void showDelGrade(MouseEvent event) {
		...
		if(list.size() == 0) {
			JOptionPane.showMessageDialog(null, "暂时没查询到学生成绩");
			dispose();
		}else {
			for (Vector v : list) {
				model.addRow(v);
